...
Usage:
    class ACTEvWrapper(BaseEvWrapper):
        ...
        def _get_checkpoint_dir(self, task_slug, config_slug, total_episodes):
            return f"policy/ACT/act_ckpt/act-{task_slug}/{config_slug}-{total_episodes}"

        def _build_eval_command(self, task_name, task_config, ckpt_dir, seed, test_num, ...):
            ...
"""
...
class BaseEvWrapper(ABC):
    ...
    def _load_ev_cfg(self, cfg_name: str) -> Dict[str, Any]:
        ...
        if not isinstance(cfg, dict) or not cfg:
            raise ValueError(f"Empty or invalid YAML config: {cfg_path}")
        return cfg
    ...
    def _resolve_eval_end_reset_to_init(self, cfg: Dict[str, Any]) -> str:
        ...
        """
        return self._to_cli_bool(cfg["END_RESET_TO_INIT"])
    ...
    def run(
        ...
        @output: [int, exit code (0 for success)]
        @scenario: [Execute evaluation from config]
        """
        try:
    ...
    def main(cls, argv: List[str]) -> int:
        ...
        if not cfg_name:
            parser.error(f"Missing cfg_name. Use: python3 _ev_wrapper.py <cfg_name>")
