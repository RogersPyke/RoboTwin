...
"""
Eval-only flow scheduler for ACT model.
...
- ACT_FLOW_END_RESET_TO_INIT from FLOW_END_RESET_TO_INIT (optional)
"""
...
MODEL_CFG_PATH = "policy/ACT/_ev_cfg/ev_tasks.yaml"
...
def start_slot_job(
    ...
    slot_env["ACT_FLOW_STEM"] = task_id
    ...
    if FLOW_SEED is not None:
        ...
        slot_env["ACT_FLOW_SEED"] = str(seed)
    if FLOW_TEST_NUM is not None:
        slot_env["ACT_FLOW_TEST_NUM"] = str(FLOW_TEST_NUM)
    else:
        slot_env["ACT_FLOW_TEST_NUM"] = str(test_num)
    if FLOW_END_RESET_TO_INIT is not None:
        slot_env["ACT_FLOW_END_RESET_TO_INIT"] = (
            "true" if FLOW_END_RESET_TO_INIT else "false"
        )
    else:
        slot_env["ACT_FLOW_END_RESET_TO_INIT"] = "true" if end_reset else "false"
