...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        self._end_reset_to_init = bool(kwags.get("END_RESET_TO_INIT", False))
        self._end_reset_completed = False
    ...
    def _execute_end_reset(self):
        """
        Execute end-of-episode reset: move arms back to origin positions.

        This method is called when END_RESET_TO_INIT is True and check_success()
        returns True for the first time. It temporarily sets eval_success to False,
        moves arms to origin, then sets eval_success to True.

        @input: None
        @output: None
        @scenario:
            Reset arms to home position after task success, before final return.
            This ensures consistent arm positions across episodes for data collection.
        """
        logger.info(
            f"[{self.__class__.__name__}] END_RESET_TO_INIT: "
            f"resetting arms to origin after task success"
        )
    ...
    def take_action(
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            if self.check_success():
                if self._end_reset_to_init and not self._end_reset_completed:
                    self._execute_end_reset()
                    self._end_reset_completed = True
                else:
                    self.eval_success = True
                    self.get_obs()
