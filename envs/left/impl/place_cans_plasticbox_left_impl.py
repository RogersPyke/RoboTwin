...
Left workspace manifest: place_cans_plasticbox, version 3 (2026-08-14)
Actors and clearance: plasticbox (static, union sweep x in [-0.42,-0.10], y in
    [-0.28,-0.02] covering the whole family appearance envelope so the box
    overlaps the can band, zero yaw, footprint-aware clearance min 0.13 m),
    can1/can2 (dynamic, each sharing the identical band x in [-0.42,-0.10], y
    in [-0.15,-0.07], zero yaw, pairwise min 0.08 m).  The rejection sampler
    keeps the two cans >= 0.08 m apart and each can >= 0.13 m clear of the box,
    so a can centre never enters the 062_plasticbox hull.
...
Pilot evidence: central-cam 30-seed pilot on version 0 0.40 (12/30 success),
    ...
    (union plasticbox sweep) 50-seed pilot 0.28 (14/50 success), manifest hash
    4e5f2e964176bc6e
"""
