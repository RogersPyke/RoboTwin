...
Left workspace manifest: place_can_basket, version 3 (2026-08-14)
Actors and clearance: basket (static, union sweep x in [-0.38,-0.12], y in
    [-0.30,0.00], zero yaw, min 0.15 m) so the basket band reaches up to touch
    the can band (y=0), can (dynamic, x in [-0.25,-0.20], y in [0,0.08], zero
    yaw, min 0.15 m).  The can band is pinned by the lying-can downward-grasp
    IK reach (y capped at 0.08) and the basket yaw is pinned at zero so the
    calibrated plate offset (+0.05, -0.04) stays valid; the widened basket
    sweep is the safe randomization lever and now overlaps the can band at the
    y=0 boundary.
...
Pilot evidence: central-cam 30-seed pilot on version 0 0.27 (8/30 success),
    ...
    pilot 0.28 (14/50 success), manifest hash ad0055fc4f784083; version 3
    (basket band reaches the can band at y=0) 50-seed pilot 0.28 (14/50
    success), manifest hash ec2e2b5aa098c862
"""
